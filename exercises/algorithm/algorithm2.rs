...
impl<T> LinkedList<T> {
    ...
    pub fn reverse(&mut self) {
        ...
        fn link_after<T>(list: &mut LinkedList<T>, mut node_ptr: NonNull<Node<T>>) {
            if list.start.is_none() {
                ...
                unsafe {
                    node_ptr.as_mut().prev = list.end;
                    node_ptr.as_mut().next = None;
                    list.end.unwrap().as_mut().next = Some(node_ptr);
                    list.end = Some(node_ptr);
                }
            }
        }
